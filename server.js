...
app.put('/api/jobs/:id', authMiddleware, async (req, res) => {
  const { data: old } = await supabase.from('jobs').select('status').eq('id', req.params.id).single();
...
app.post('/api/webhooks/sms', async (req, res) => {
  ...
  await supabase.from('messages').insert({ contact_name, contact_phone: From, contact_type, direction: 'in', body: Body, read: false });
...
app.listen(PORT, () => console.log(`FieldPro CRM running on port ${PORT}`));
