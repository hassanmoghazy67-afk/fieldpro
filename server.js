...
async function runAlertChecks() {
  ...
  if (await alertsEnabled(settings, 'alert_appointment_reminder')) {
    ...
    for (const job of upcoming || []) {
      ...
      if (job.tech_id && await appointmentReminderAppliesTo(job)) await sendAppointmentReminderTicket(job);
      // Fire any workflows built on the new "Appointment coming up" trigger, so
      // you can add your own actions (text the customer, add a tag, notify)
      // on top of the built-in tech reminder.
      runEventWorkflows('appointment_due', job).catch(() => {});
      // also drain any delayed workflow actions that are now due
