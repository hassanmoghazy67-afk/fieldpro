...
async function sendAppointmentReminderTicket(job) {
  ...
  const data = {
    ...
    call_extension: fieldVisibility.call_extension !== false ? `Call via business line: ${settings.twilio_number || process.env.TWILIO_PHONE_NUMBER}` : ''
  };
...
async function runAlertChecks() {
  ...
  // 6) Mark techs "unavailable" if no SMS activity in a while, so the next message
