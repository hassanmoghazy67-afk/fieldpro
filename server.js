...
// ─── REAL SMS SUBSCRIPTION FLOW (keyword-based opt-in) ─────────────────────────
// This is a genuine two-step flow, not just wording: a new customer's first
// contact is a real welcome message inviting them to text JOIN; only after they
// reply JOIN do they get marked subscribed and receive a real confirmation
// message. Until they reply JOIN, they're still sent their actual job/receipt
// content (they reached out for a real service need, withholding that would be
// bad business) — but every message keeps reminding them how to subscribe or
// stop, so the invitation stays live until they act on it one way or the other.

const SMS_WELCOME_MESSAGE_SUFFIX = `\n\nMsg & data rates may apply. Reply JOIN to subscribe to SMS updates, or STOP to opt out.`;
const SMS_CONFIRMATION_MESSAGE = `You're subscribed to SMS updates from Express Lock&Key. Msg & data rates may apply. Reply STOP to unsubscribe at any time.`;

// Called right after a job is created/updated with a customer phone — sends the
// real welcome invite exactly once per customer, the first time we ever have
// their number on file.
async function sendWelcomeMessageIfNewCustomer(phone, customerId) {
  if (!phone) return;
  try {
    const { data: customer } = await supabase.from('customers').select('id,sms_subscription_status,sms_welcome_sent_at').eq('id', customerId).single();
    if (!customer || customer.sms_welcome_sent_at) return; // already welcomed, don't repeat it
    await supabase.from('customers').update({ sms_subscription_status: 'welcome_sent', sms_welcome_sent_at: new Date().toISOString() }).eq('id', customerId);
    // The welcome framing itself happens by appending the suffix to whatever the
    // first real message is (handled in appendComplianceFooterIfFirstContact) —
    // this function just marks that the welcome has now been sent.
  } catch(e) { console.error('sendWelcomeMessageIfNewCustomer error:', e.message); }
}

// Appends the appropriate disclosure to a customer-facing SMS: the full
// welcome+JOIN invite on their very first message, then a lighter STOP-only
// reminder afterward until they actually subscribe, then nothing once subscribed.
async function appendComplianceFooterIfFirstContact(phone, body) {
  try {
    const { data: customer } = await supabase.from('customers').select('id,sms_subscription_status,sms_welcome_sent_at').eq('phone', phone).single();
    if (!customer) return `${body}${SMS_WELCOME_MESSAGE_SUFFIX}`; // no record at all — treat as first contact
    if (customer.sms_subscription_status === 'subscribed') return body; // already confirmed — no need to keep prompting
    if (!customer.sms_welcome_sent_at) {
      await supabase.from('customers').update({ sms_subscription_status: 'welcome_sent', sms_welcome_sent_at: new Date().toISOString() }).eq('id', customer.id);
      return `${body}${SMS_WELCOME_MESSAGE_SUFFIX}`;
    }
    return `${body}\n\nReply JOIN to subscribe, STOP to unsubscribe.`;
  } catch(e) {
    return `${body}${SMS_WELCOME_MESSAGE_SUFFIX}`;
  }
}

// Called from the inbound SMS webhook — checks if an incoming message is the
// JOIN keyword, and if so, marks the customer subscribed and sends the real
// confirmation message back.
async function handleJoinKeywordIfMatched(phone, bodyText) {
  const normalized = (bodyText || '').trim().toUpperCase();
  if (normalized !== 'JOIN' && normalized !== 'SUBSCRIBE') return false;
  try {
    const { data: customer } = await supabase.from('customers').select('id').eq('phone', phone).single();
    if (customer) {
      await supabase.from('customers').update({ sms_subscription_status: 'subscribed', sms_subscribed_at: new Date().toISOString() }).eq('id', customer.id);
    }
    await twilioClient.messages.create({ body: SMS_CONFIRMATION_MESSAGE, from: process.env.TWILIO_PHONE_NUMBER, to: phone });
    return true;
  } catch(e) { console.error('handleJoinKeywordIfMatched error:', e.message); return false; }
}
...
app.post('/api/webhooks/sms', async (req, res) => {
  try {
    ...
    // Real keyword opt-in: if this is a JOIN/SUBSCRIBE reply from a customer,
    // handle it and stop — no need to run the rest of the inbound-message logic
    // (closing detection, job matching, etc.) for what is purely a subscription
    // confirmation, not a job-related message.
    if (customer) {
      const joinHandled = await handleJoinKeywordIfMatched(From, bodyTrim);
      if (joinHandled) {
...
// Public — no auth. Lets the standalone SMS opt-in page show the real business
// name without needing a login, so it never goes stale if the name changes.
app.get('/api/config/business-name', async (req, res) => {
  try {
    const { data } = await supabase.from('settings').select('value').eq('key', 'business_name').single();
    res.json({ name: data?.value || 'Express Lock&Key' });
  } catch(e) { res.json({ name: 'Express Lock&Key' }); }
});

// Public — no auth. Real opt-in submissions for SMS consent, used as the actual
// documented proof of consent Twilio/carriers require for A2P 10DLC registration.
// Every submission is timestamped and stored — this is the genuine record, not
// just a UI gesture.
app.post('/api/sms-optin', async (req, res) => {
  try {
    ...
    if (error) return res.status(500).json({ error: error.message });
    // Also create/update a customer record so this person shows up in the CRM
