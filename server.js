...
app.post('/api/jobs', authMiddleware, async (req, res) => {
  const { title, customer_id, customer_name, phone, address, job_type, car_make_model, car_year, tech_id, tech_name, priority, status, notes, job_date, scheduled_date, zip_code } = req.body;
...
async function sendAppointmentReminderTicket(job) {
  ...
  const data = {
    ...
    scheduled_time: fieldVisibility.scheduled_time !== false ? new Date(job.scheduled_date).toLocaleString() : '',
...
app.post('/api/webhooks/voice', async (req, res) => {
  ...
  // Tech calling in — offer the extension IVR so they can reach the customer
  // without ever seeing the customer's real number.
  const { data: tech } = await supabase.from('users').select('id,name').eq('phone', From).eq('role', 'tech').single();
  if (tech) {
    return res.send(`<Response><Gather numDigits="3" timeout="8" action="/api/webhooks/voice/extension" method="POST"><Say>Welcome to Field Pro. Please enter the 3 digit extension for your job, followed by the pound key.</Say></Gather><Say>No input received. Goodbye.</Say></Response>`);
  }

  // Check if caller is a known customer — if so, find their tech (existing behavior)
  const { data: customer } = await supabase.from('customers').select('name').eq('phone', From).single();
  let techPhone = null;
  if (customer) {
    ...
    if (job?.tech_id) {
      ...
      techPhone = t?.phone;
      await createNotification('inbound_call', `Inbound call from ${customer.name}`, `Customer calling back — routing to ${t?.name || 'tech'}`);
    }
  }
  if (techPhone) {
