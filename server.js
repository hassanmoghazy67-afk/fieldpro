...
app.get('/api/users', authMiddleware, async (req, res) => {
  const { data, error } = await supabase.from('users').select('id,name,email,role,phone,color,initials,status,can_view_all_jobs,can_edit_jobs,can_view_finance,use_masking,direct_phone,default_commission_type,default_commission_value,commission_rules,qualified_job_types,perm_view_jobs,perm_edit_jobs,perm_delete_jobs,perm_view_customers,perm_view_sms,perm_view_booking,perm_view_finance_tools,perm_view_total_revenue,perm_view_reports,perm_view_workflows,perm_view_ai_assistant,perm_view_closing_messages,perm_manage_techs,perm_view_settings,created_at').order('created_at');
...
app.post('/api/users', authMiddleware, adminOnly, async (req, res) => {
  const { name, email, password, role, phone, color, default_commission_type, default_commission_value, commission_rules, use_masking } = req.body;
  if (!name || !email || !password) return res.status(400).json({ error: 'Name, email, password required' });
  const hash = await bcrypt.hash(password, 10);
  const initials = name.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();
  const { data, error } = await supabase.from('users').insert({
    name, email, password_hash: hash, role: role || 'tech', phone, color: color || '#1e6fff', initials,
...
app.put('/api/users/:id', authMiddleware, adminOnly, async (req, res) => {
  const { name, role, phone, color, status, can_view_all_jobs, can_edit_jobs, can_view_finance, default_commission_type, default_commission_value, commission_rules, use_masking, qualified_job_types } = req.body;
...
app.put('/api/jobs/:id', authMiddleware, requirePerm('perm_edit_jobs'), async (req, res) => {
  ...
  if (update.tech_id && update.tech_id !== old?.tech_id) {
    // Block assigning a job to a tech who isn't qualified for this job type.
    // Empty/unset qualified_job_types means "no restriction" so existing techs
    // aren't suddenly locked out until an admin explicitly configures them.
    const jobType = old?.job_type;
    if (jobType) {
      const { data: tech } = await supabase.from('users').select('name,qualified_job_types').eq('id', update.tech_id).single();
      const qualified = tech?.qualified_job_types || [];
      if (qualified.length > 0 && !qualified.includes(jobType)) {
        return res.status(409).json({ error: `${tech.name} is not qualified for "${jobType}" jobs. Update their qualifications in Technicians settings, or choose a different tech.` });
      }
    }
  ...
  if (update.tech_name && update.tech_name !== old?.tech_name) {
    ...
    if (update.tech_id) {
      ...
      if (tech?.phone) {
        ...
        try { await twilioClient.messages.create({ body: msg, from: process.env.TWILIO_PHONE_NUMBER, to: tech.phone }); } catch(e) {}
      }
    }
...
app.get('/api/bookings', authMiddleware, requirePerm('perm_view_booking'), async (req, res) => {
...
// ─── REAL AVAILABILITY (slot-based online booking) ─────────────────────────────
// Returns genuinely open slots for a given date — checks actual scheduled jobs
// against total active techs, not a fake hourly list. A slot is "full" once every
// active tech already has a job at that time.
app.get('/api/booking/availability', async (req, res) => {
  const date = req.query.date;
  if (!date) return res.status(400).json({ error: 'Date required' });
  const { data: s } = await supabase.from('settings').select('key,value');
  const settings = {}; s?.forEach(x => settings[x.key] = x.value);
  if (settings.google_booking_enabled !== 'true') return res.json({ enabled: false, slots: [] });

  const startHour = parseInt((settings.booking_business_hours_start || '08:00').split(':')[0]);
  const endHour = parseInt((settings.booking_business_hours_end || '20:00').split(':')[0]);
  const slotDuration = parseInt(settings.google_booking_slot_duration || '60');

  const { data: techs } = await supabase.from('users').select('id').eq('role', 'tech');
  const techCount = Math.max(techs?.length || 1, 1);

  // All jobs scheduled that day, regardless of status, count as occupying a tech's time
  const dayStart = new Date(date + 'T00:00:00');
  const dayEnd = new Date(date + 'T23:59:59');
  const { data: scheduledJobs } = await supabase.from('jobs').select('scheduled_date').gte('scheduled_date', dayStart.toISOString()).lte('scheduled_date', dayEnd.toISOString()).not('status', 'in', '("cancelled")');
  ...
  for (let h = startHour; h < endHour; h += (slotDuration / 60)) {
    ...
    const slotTime = new Date(`${date}T${String(hour).padStart(2,'0')}:${String(minute).padStart(2,'0')}:00`);
    if (slotTime <= now) continue; // never offer a past slot
    const bookedCount = (scheduledJobs || []).filter(j => {
      const jt = new Date(j.scheduled_date);
      return Math.abs(jt.getTime() - slotTime.getTime()) < (slotDuration * 60000) / 2;
    }).length;
    if (bookedCount < techCount) {
      slots.push({ time: slotTime.toISOString(), label: slotTime.toLocaleTimeString('en-US', { timeZone: BUSINESS_TZ, hour: 'numeric', minute: '2-digit' }), spots_left: techCount - bookedCount });
    }
  }
  res.json({ enabled: true, slots, days_ahead: parseInt(settings.google_booking_days_ahead || '14') });
});
...
app.post('/api/book', async (req, res) => {
  ...
  const { data: s } = await supabase.from('settings').select('key,value');
  const settings = {}; s?.forEach(x => settings[x.key] = x.value);
  const autoConfirm = settings.booking_auto_confirm === 'true' && settings.google_booking_enabled === 'true';
  let createdJobId = null;

  // Real slot-based booking + auto-confirm enabled — skip the manual dispatch
  // step entirely and create the job immediately.
  if (type === 'scheduled' && scheduled_time && autoConfirm) {
    let customer = await supabase.from('customers').select('*').eq('phone', phone).single();
    let customerRow = customer.data;
    if (!customerRow) {
      const created = await supabase.from('customers').insert({ name: customer_name, phone, email, address }).select().single();
      customerRow = created.data;
    }
    const count = await supabase.from('jobs').select('id', { count: 'exact' });
    const num = String((count.count || 0) + 1).padStart(3, '0');
    createdJobId = `JOB-${num}`;
    await supabase.from('jobs').insert({
      id: createdJobId, title: job_type || 'Service Request', customer_id: customerRow?.id, customer_name,
      phone, address, job_type, car_make_model, car_year, status: 'new', priority: 'med',
      job_date: todayInBusinessTz(), scheduled_date: scheduled_time, notes, source: null
    });
    await supabase.from('bookings').update({ status: 'confirmed', job_id: createdJobId }).eq('id', data.id);
    runEventWorkflows('new_booking', { id: createdJobId, customer_name, job_type }).catch(() => {});
  }

  // Auto SMS customer acknowledgement
  try {
    const wording = createdJobId ? 'your appointment is confirmed!' : (settings.booking_auto_confirm === 'true' ? 'your booking is confirmed!' : 'we received your request and will confirm shortly.');
    await twilioClient.messages.create({
      body: `Hi ${customer_name}, ${wording} — ${settings.business_name || 'Express Lock&Key'}`,
      from: process.env.TWILIO_PHONE_NUMBER, to: phone
    });
  } catch(e) { console.log('SMS error:', e.message); }
  res.json({ success: true, booking_id: data.id, job_id: createdJobId });
});
...
// Replaces the old dead 'automations' engine. This actually runs whenever a real
// event happens (new_job, job_assigned, status_change), checking each active
// workflow's trigger_condition against the job. Conditions now support tags and
// status, not just arbitrary fields, since those are the two most useful checks
// dispatchers actually use day to day.
...
app.get('/api/analytics/yearly-trend', authMiddleware, adminOnly, async (req, res) => {
  ...
  const { data: invoices } = await supabase.from('invoices').select('id,total,status,invoice_date,tech_id,job_id');
