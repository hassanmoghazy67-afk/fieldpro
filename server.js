...
app.post('/api/webhooks/sms', async (req, res) => {
  ...
  if (tech) {
    ...
    if (job) {
      ...
      if (CLOSING_PATTERN.test(bodyTrim) && bodyTrim.length < 200) {
        ...
        const amtMatch = bodyTrim.match(/(\d{2,4})\s*\$|\$\s*(\d{2,4})/);
        closing_amount = amtMatch ? parseFloat(amtMatch[1] || amtMatch[2]) : null;
      }
    }
  ...
  await supabase.from('messages').insert({ contact_name, contact_phone: From, contact_type, direction: 'in', body: Body, read: false, job_id, is_closing_attempt, closing_amount });
  ...
  if (is_closing_attempt) {
    await createAlert('closing_message', {
      ...
      data: { message: bodyTrim, amount: closing_amount }
    });
  }
