...
function broadcastNudge(type, data) {
  // The channel still carries no business content — at most an id, so the
  // client can refetch that ONE record over its own authenticated request
  // instead of re-downloading the entire jobs table on every change.
  const body = data && typeof data === 'object' ? JSON.stringify(data) : '{}';
  const payload = `event: ${type}\ndata: ${body}\n\n`;
  for (const client of sseClients) {
    try { client.write(payload); client.flush?.(); }   // flush: never let an event linger in a buffer
...
async function generateJobId() {
  ...
  // PERF NOTE: this deliberately fetches ALL JOB-% ids and is left alone. A
  // bounded ORDER BY id DESC window is NOT safe here: ids sort as text, so
  // 'JOB-999' sorts above 'JOB-5797' and a LIMIT would miss the true numeric
  // max and hand out a colliding id — the exact bug fixed above. The correct
  // optimisation is a SQL-side max on the numeric part; do that with the
  // schema sync work. The filter returns few rows anyway (most historical
  // imports use other id schemes), so this is not a hot path.
...
// Gzip everything. The app shell (~426KB HTML) and the large JSON payloads
// (jobs, messages, invoices) compress by roughly 70-85%, which is the single
// biggest transfer-time win for boot and every subsequent data fetch.
const compression = require('compression');
// COMPRESSION MUST NOT TOUCH THE LIVE EVENT STREAM.
//
// This was the reason background notifications never arrived. compression()
// buffers the response body, and an SSE stream is a response that never ends —
// so every pushed event sat in the compression buffer instead of being
// delivered. The browser's connection stayed open and looked healthy while
// receiving nothing, so alerts only appeared when you returned to the tab and
// the polling fallback ran. Measured: with compression an event NEVER arrived
// in 3 seconds; bypassing it, the same event arrived in 301ms.
//
// A content-type filter does NOT work here — compression() decides before the
// route handler has set Content-Type — so the stream path is skipped outright.
const _compress = compression();
app.use((req, res, next) => {
  if (req.path === '/api/events') return next();   // never compress the SSE stream
  return _compress(req, res, next);
});
...
app.get('/api/events', async (req, res) => {
  ...
  // no-transform tells any proxy in front of us (Railway's edge, a CDN, a
  // corporate proxy) not to buffer or re-encode the stream either.
  res.set({ 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache, no-transform', Connection: 'keep-alive', 'X-Accel-Buffering': 'no' });
  res.flushHeaders?.();
  res.write('event: connected\ndata: {}\n\n');
  res.flush?.();
  sseClients.add(res);
  const heartbeat = setInterval(() => { try { res.write(': ping\n\n'); res.flush?.(); } catch(e) {} }, 25000);
...
app.post('/api/webhooks/sms-status', twilioWebhookAuth, async (req, res) => {
  try {
    ...
    if (MessageSid && MessageStatus) {
      ...
      // Carry the actual change so clients can patch one row instead of
      // refetching. delivery_status changes don't move created_at, so an
      // incremental ?since= poll would otherwise never see them.
      if (data && data.length) broadcastNudge('message_status', { id: data[0].id, delivery_status: MessageStatus });
...
// PERF: this used to page STRICTLY sequentially — ask for 1,000 rows, wait for
// the reply, then ask for the next 1,000. With ~5,900 jobs that's six
// back-to-back round trips to Supabase for a single request, and every one of
// them pays the full network distance between Railway and Supabase.
//
// Now it requests a batch of pages in parallel and stops as soon as a page
// comes back short (which means we've reached the end). Six sequential trips
// become two parallel ones. Ordering is preserved because each page is a fixed
// range against the same ordered query, and results are concatenated in range
// order regardless of which reply arrives first.
async function fetchAllRows(buildQuery) {
  const PAGE = 1000;
  const BATCH = 4; // pages fetched concurrently per round
  let all = [], from = 0, done = false;
  while (!done) {
    const starts = [];
    for (let i = 0; i < BATCH; i++) starts.push(from + i * PAGE);
    const results = await Promise.all(
      starts.map(start => buildQuery().range(start, start + PAGE - 1))
    );
    for (const r of results) {
      if (r.error) throw new Error(r.error.message);
      const d = r.data || [];
      all = all.concat(d);
      if (d.length < PAGE) { done = true; break; } // short page = end of data
    }
    from += BATCH * PAGE;
  }
  return all;
...
app.get('/api/jobs', authMiddleware, async (req, res) => {
  try {
  // ?limit=N returns only the N most-recent jobs in a SINGLE query — used for a
  // fast boot (the recent jobs are all any page needs to render immediately).
  // Without limit, fetchAllRows pages through the entire table (5,900+ rows,
  // ~4.5s), which is what made startup feel frozen. The full set is loaded
  // lazily in the background after the UI is already interactive.
  const limit = parseInt(req.query.limit);
  if (limit && limit > 0) {
    let q = supabase.from('jobs').select('*').order('created_at', { ascending: false }).limit(Math.min(limit, 2000));
    if (req.user.role === 'tech') q = q.eq('tech_id', req.user.id);
...
app.post('/api/jobs', authMiddleware, async (req, res) => {
  try {
    ...
    if (phone) {
      ...
      if (norm.length === 10) {
        // PERF: try an indexed exact match first (idx_cust_phone). Stored phone
        // formats are inconsistent across legacy imports, so an exact match can
        // miss — if it does, fall back to the original full-scan normalise-and-
        // compare so matching behaviour is byte-for-byte unchanged. Fast path
        // handles the common case; correctness never depends on it.
        let hit = null;
        const { data: exactC } = await supabase.from('customers').select('id,phone').eq('phone', phone).limit(1);
        if (exactC && exactC.length) hit = exactC[0];
        if (!hit) {
          const { data: existingC } = await supabase.from('customers').select('id,phone');
          hit = (existingC || []).find(c => String(c.phone || '').replace(/\D/g, '').slice(-10) === norm) || null;
        }
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      ({ data, error } = await supabase.from('jobs').insert({
        ...
        tech_id, tech_name, priority: priority || 'med', status: status || 'unassigned', notes, job_date, scheduled_date,
...
app.put('/api/jobs/:id', authMiddleware, requirePerm('perm_edit_jobs'), async (req, res) => {
  try {
    ...
    // Setting a job back to "Unassigned" (or legacy "new") clears the tech — a
    // job can't logically be both unassigned and have a technician. This runs
    // no matter how the status was changed (ticket, drawer, quick menu, bulk).
    if (update.status === 'unassigned' || update.status === 'new') {
      update.tech_id = null;
      update.tech_name = null;
    ...
    // Rescheduling clears the reminder flag — otherwise a job moved from
    // Tuesday to Friday keeps `appointment_reminder_sent = true` and NEVER
    // reminds anyone about the new time.
    if (update.scheduled_date && update.scheduled_date !== old?.scheduled_date) {
      supabase.from('jobs').update({ appointment_reminder_sent: false }).eq('id', req.params.id)
        .then(() => {}, err => console.error('reset reminder flag failed:', err?.message));
    }
    ...
    if (error) return res.status(500).json({ error: error.message });
    // RESPOND FIRST. Everything below — history, notifications, extension
    // allocation, workflows, audit SMS — is bookkeeping the dispatcher does not
    // wait on. allocateExtension() in particular calls expireStaleExtensions()
    // and then hunts for a free random extension with repeated DB checks, which
    // has no business sitting between "assign Mike" and the screen updating.
    // The push below carries the full updated job so clients patch one row
    // instead of refetching the table.
    res.json(data);
    broadcastNudge('job_change', { id: data.id, job: data });
    broadcastNudge('jobs_changed', { id: data.id, job: data });
    ...
    if (update.status && update.status !== old?.status) {
      ...
      // A cancelled or done job should never keep generating "resend / tech
      // available / stale" alerts — that was the "it keeps telling me to resend
      // a cancelled job" bug. Auto-resolve every open alert tied to this job the
      // moment it leaves the active pipeline.
      if (update.status === 'done' || update.status === 'cancelled') {
        await supabase.from('alerts').update({ resolved: true, resolved_at: new Date().toISOString(), resolved_by: 'auto: job ' + update.status })
          .eq('job_id', req.params.id).eq('resolved', false).then(r => r, () => {});
      }
    ...
    if (update.tech_name && update.tech_name !== old?.tech_name) {
      ...
      runEventWorkflows('job_assigned', data).catch(() => {});
      // The extension is allocated after the response, so tell the clients
      // about it once it exists rather than making them poll for it.
      try {
        const { data: fresh } = await supabase.from('jobs').select('*').eq('id', req.params.id).single();
        if (fresh) broadcastNudge('job_change', { id: fresh.id, job: fresh });
      } catch(e) {}
    }
    })().catch(err => console.error('/api/jobs/:id background work failed:', err.message));
  } catch(e) {
...
// NOTE: the older /api/invoices/:id/send handler that used to live here has
// been REMOVED. Express matches the FIRST route registered for a path, so it
// was shadowing the newer document-sending route further down — and it looked
// the phone up only via invoices.customer_id, which is null on invoices built
// from a ticket. That produced "No customer phone on file" even when the job
// clearly had a number. There is now exactly one handler for this path.
...
app.get('/api/messages', authMiddleware, requirePerm('perm_view_sms'), async (req, res) => {
  try {
  // INCREMENTAL MODE. Every inbound SMS used to make every open tab re-download
  // the ENTIRE messages table — thousands of rows, paged out of Postgres, just
  // to learn about one new row. With ?since=<iso> we return only what's newer,
  // which is normally a single message or an empty array. The client still does
  // a periodic full sync to pick up read/archive flags that don't change
  // created_at, so nothing goes stale.
  const since = req.query.since;
  if (since && !isNaN(Date.parse(since))) {
    const { data, error } = await supabase.from('messages').select('*')
      .gt('created_at', new Date(Date.parse(since)).toISOString())
...
app.post('/api/jobs/:id/send-to-tech', authMiddleware, smsSendLimiter, async (req, res) => {
  try {
    ...
    broadcastNudge('jobs_changed', { id: req.params.id });
...
app.post('/api/messages/send', authMiddleware, smsSendLimiter, async (req, res) => {
  const { contact_name, contact_phone, contact_type, body, job_id: clientJobId } = req.body;
  if (!contact_phone || !body) return res.status(400).json({ error: 'Phone and message required' });
  try {
    // DISPATCH-CRITICAL PATH. This used to run strictly serially:
    //   Twilio API -> findRelevantJobForPhone (up to 3 Supabase trips)
    //   -> insert -> await logAudit -> respond
    // The dispatcher's Send button therefore waited on Twilio's API, a job
    // lookup they don't care about, and an audit write. That's the multi-second
    // pause. Now the ONLY thing between the click and the response is a single
    // insert; everything else runs after the response has already gone out.
    const { data, error } = await supabase.from('messages').insert({
      contact_name, contact_phone, contact_type, direction: 'out', body, read: true,
      job_id: clientJobId || null,
      sent_by_user_id: req.user.id, sent_by_name: req.user.name,
      delivery_status: 'queued'
    }).select().single();
    if (error) throw new Error(error.message);

    res.json(data);                 // dispatcher is unblocked HERE
    broadcastNudge('new_message');  // other tabs see it immediately too

    // ─── everything below is off the critical path ───
    (async () => {
      try {
        const msg = await twilioClient.messages.create({
          body, from: process.env.TWILIO_PHONE_NUMBER, to: contact_phone,
          statusCallback: `${PUBLIC_BASE_URL}/api/webhooks/sms-status`
        });
        const patch = { twilio_sid: msg.sid, delivery_status: 'sent' };
        // The browser already knows the conversation's job, so it sends job_id
        // and we skip the lookup entirely. Only fall back when it didn't.
        if (!clientJobId) {
          const jid = await findRelevantJobForPhone(contact_phone).catch(() => null);
          if (jid) patch.job_id = jid;
        }
        await supabase.from('messages').update(patch).eq('id', data.id);
        broadcastNudge('message_status', { id: data.id, delivery_status: 'sent', twilio_sid: msg.sid, job_id: patch.job_id || data.job_id || null });
      } catch (err) {
        console.error('outbound SMS failed for message', data.id, ':', err.message);
        await supabase.from('messages').update({ delivery_status: 'failed' }).eq('id', data.id).catch(() => {});
        broadcastNudge('message_status', { id: data.id, delivery_status: 'failed' });
      }
      logAudit('message', data.id, req.user.id, req.user.name, 'sent', `Sent SMS to ${contact_name || contact_phone}`).catch(() => {});
    })();
  } catch (e) {
    console.error('/api/messages/send error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
app.post('/api/webhooks/sms', twilioWebhookAuth, async (req, res) => {
  try {
    ...
    //
    // Carry WHO it's from and a short preview. A background tab can then raise
    // the desktop notification straight from this event, without waiting on a
    // fetch — browsers throttle background timers to roughly once a minute (and
    // freeze them entirely after a few minutes), so anything routed through a
    // timer arrives far too late to be useful as an alert.
    broadcastNudge('new_message', {
      from: contact_name || From,
      preview: String(Body || '').slice(0, 120) || (inboundMediaUrl ? 'Sent an attachment' : ''),
      phone: From
    });
    res.set('Content-Type', 'text/xml');
    ...
    (async () => {
      try {
        if (tech) {
          ...
          if (wasUnavailable) {
            // Only reference genuinely actionable jobs — assigned/in-progress,
            // never done/cancelled. If the tech has nothing open, this is just a
            // low-key "available" note, not a "resend the job" prompt (which was
            // firing even for cancelled jobs before).
            ...
            await createAlert('tech_available', {
              ...
              body: openList.length
                ? `${tech.name} is now available and still has ${openList.length} active job(s) in progress.`
                : `${tech.name} is now available for new assignments.`,
              tech_id: tech.id, tech_name: tech.name, severity: 'info',
              data: { open_jobs: openList }
            });
...
// PERF: settings are read on nearly every alert, workflow action and template
// render, but change maybe once a week. Cache for 60s — same pattern as the
// auth cache. Any write path must call invalidateSettingsCache() so admin
// edits apply immediately rather than up to a minute later.
...
async function getSettingsMap() {
  ...
  _settingsCache = obj; _settingsCacheAt = Date.now();
  return obj;
}
...
// The built-in reminder honours the same tech scoping as workflows, stored in
// settings so it's editable in the app: a comma-separated list of tech ids in
// `reminder_techs` (empty = everyone) and `reminder_exclude_techs`.
async function appointmentReminderAppliesTo(job) {
  try {
    const settings = await getSettingsMap();
    const only = String(settings.reminder_techs || '').split(',').map(v => v.trim()).filter(Boolean);
    const excl = String(settings.reminder_exclude_techs || '').split(',').map(v => v.trim()).filter(Boolean);
    const t = String(job.tech_id);
    if (only.length && !only.includes(t)) return false;
    if (excl.length && excl.includes(t)) return false;
    return true;
  } catch(e) { return true; }   // never let a settings hiccup silence reminders
}
...
async function sendAppointmentReminderTicket(job) {
  ...
  if (!tech?.phone) { console.error(`[reminders] ${job.id}: tech ${job.tech_name || job.tech_id} has no phone on file`); return false; }
  let fieldVisibility = {};
  ...
  // The call extension is a NICE-TO-HAVE on the reminder. If allocation fails
  // (none free, DB hiccup), send the reminder without it rather than losing the
  // reminder entirely — the tech still needs to know about the appointment.
  let ext = null;
  if (fieldVisibility.call_extension !== false) {
    try { ext = await allocateExtension(job.id); }
    catch(e) { console.error(`[reminders] extension allocation failed for ${job.id} (sending without it):`, e.message); }
  }
  const data = {
    job_id: job.id,
  ...
  // (sent + logged below so the reminder is visible in the tech's thread)
  try {
    const sentRem = await twilioClient.messages.create({ body: message, from: process.env.TWILIO_PHONE_NUMBER, to: tech.phone });
    await logOutboundToThread({
      phone: tech.phone, body: message, job, twilio_sid: sentRem?.sid,
      name: job.tech_name || tech.name || 'Tech', contact_type: 'tech',
      sent_by: 'Appointment reminder'
    });
    return true;   // caller logs whether it actually went out
  } catch(e) {
    console.error('Appointment reminder SMS failed for', job.id, ':', e.message);
    try {
      await createAlert('workflow_failed', {
        title: `Appointment reminder failed — ${job.id}`,
        body: `Could not text the reminder to ${job.tech_name || 'the tech'}: ${e.message}`,
        job_id: job.id, severity: 'warning'
      });
    } catch(_) {}
  }
}
...
// Maps each alert TYPE to the settings toggle that governs it. createAlert uses
// this so the toggle is enforced at the single choke point every alert passes
// through — no caller can bypass it.
const ALERT_TYPE_TOGGLE = {
  tech_available: 'alert_tech_available',
  tech_no_response: 'alert_tech_no_response',
  closing_message: 'alert_closing_mismatch',
  stale_job: 'alert_stale_job',
  appointment_reminder: 'alert_appointment_reminder'
};

let _lastAlertError = null; // surfaced by /api/alerts/test so the exact DB error is visible without Railway logs
async function createAlert(type, { title, body, job_id = null, tech_id = null, tech_name = null, severity = 'info', data = {}, dedupeWindowMinutes = 30, resolved = false }) {
  // ── THE RULE, ENFORCED HERE AND ONLY HERE ──
  // Alerts toggled off → NOTHING gets created, no matter which code path calls
  // this. Previously some callers (the SMS webhook's tech_available and
  // closing_message alerts, and the manual test) skipped the toggle check
  // entirely, which is exactly why alerts kept appearing after being turned
  // off. Now createAlert itself is the single gate: it re-reads settings live
  // and refuses if the master switch is off OR this specific alert type is off.
  // The only exception is the explicit admin test alert (type 'test'), which is
  // a deliberate manual action, not an automated trigger.
  if (type !== 'test') {
    const settings = await getSettingsMap();
    if (settings.alerts_master === 'false') return null; // master kill-switch
    const toggleKey = ALERT_TYPE_TOGGLE[type];
    if (toggleKey && settings[toggleKey] === 'false') return null; // this type is off
  }
  let dedupeQuery = supabase.from('alerts').select('id').eq('type', type).eq('resolved', false);
  if (job_id) dedupeQuery = dedupeQuery.eq('job_id', job_id);
  else if (tech_id) dedupeQuery = dedupeQuery.eq('tech_id', tech_id);
  const { data: existing } = await dedupeQuery.limit(1);
  if (existing && existing.length) {
    // Refresh the existing alert's content instead of stacking a new one.
    // Deliberately does NOT touch updated_at (that column isn't guaranteed to
    // exist on the alerts table in prod) — updating body/title/data is enough
    // to keep the single alert current.
    await supabase.from('alerts').update({ title, body, data, severity })
      .eq('id', existing[0].id).then(r => r, () => {});
    return null;
  }
  ...
  // Some alerts are purely informational (e.g. a customer accepted an estimate)
  // — there is nothing for a dispatcher to DO. Those are inserted already
  // resolved so they notify without joining the action queue.
  const row = { type, job_id, tech_id, tech_name, title, body, data, severity };
  if (resolved === true) row.resolved = true;
  const { data: alert, error: insErr } = await supabase.from('alerts').insert(row).select().single();
  if (insErr) {
    _lastAlertError = insErr.message;
    console.error('createAlert INSERT FAILED (' + type + '):', insErr.message, '— run the latest migration; the alerts table is likely missing columns or an RLS policy');
    return null;
  }
  _lastAlertError = null;
...
async function runAlertChecks() {
  runDueWorkflowActions().catch(err => console.error('workflow queue sweep failed:', err.message));
  const settings = await getSettingsMap();
  // Master kill-switch: if all alerts are off, don't even run the checks. (Also
  // enforced inside createAlert, but skipping the whole pass saves the queries.)
  if (settings.alerts_master === 'false') return;
  ...
  // 2) Stale job — open with no movement for N hours. Recurs until closed or
  // cancelled, BUT stops once a job is genuinely abandoned (older than the max
  // age), so week-old in-progress jobs that will never be closed don't nag
  // forever. Default max age 48h; configurable via alert_stale_job_max_hours.
  if (await alertsEnabled(settings, 'alert_stale_job')) {
    const staleMaxHours = parseFloat(settings.alert_stale_job_max_hours || '48');
    ...
    for (const job of openJobs || []) {
      ...
      if (hoursSince > staleMaxHours) continue; // abandoned, not actionable — stop alerting
  ...
  if (await alertsEnabled(settings, 'alert_appointment_reminder')) {
    ...
    console.log(`[reminders] window ${now.toISOString()} → ${windowEnd.toISOString()} — ${(upcoming || []).length} job(s) due`);
    for (const job of upcoming || []) {
      // EVERY job is isolated. Previously a single failure here — most often
      // allocateExtension() finding no free extension — threw straight out of
      // the loop, which aborted the WHOLE sweep: no SMS, the
      // appointment_reminder_sent flag never set, and every later check in the
      // run skipped. It then failed identically every 5 minutes forever, which
      // is exactly the "reminders just don't work" symptom.
      try {
      ...
      } catch(e) { console.error(`[reminders] alert failed for ${job.id}:`, e.message); }

      let sent = false, why = '';
      try {
        if (!job.tech_id) why = 'no tech assigned — the reminder texts the tech';
        else if (!(await appointmentReminderAppliesTo(job))) why = 'excluded by reminder_techs settings';
        else { sent = await sendAppointmentReminderTicket(job); if (!sent) why = 'tech has no phone number on file'; }
      } catch(e) {
        why = 'send failed: ' + e.message;
        console.error(`[reminders] ${job.id} send failed:`, e.message);
        try {
          await createAlert('workflow_failed', {
            title: `Appointment reminder failed — ${job.id}`,
            body: `Could not send the reminder for ${job.id}: ${e.message}`,
            job_id: job.id, severity: 'warning'
          });
        } catch(_) {}
      }
      console.log(`[reminders] ${job.id}: ${sent ? 'SMS sent to ' + (job.tech_name || 'tech') : 'NOT sent — ' + why}`);

      try { runEventWorkflows('appointment_due', job).catch(() => {}); } catch(e) {}

      // Mark it handled either way. Leaving the flag false would retry the same
      // failing job every 5 minutes indefinitely; the alert above is the record
...
app.post('/api/alerts/test', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (!alert) return res.status(500).json({ error: 'createAlert failed — exact database error: ' + (_lastAlertError || 'none captured; see Railway logs') });
    res.json({ success: true, alert_id: alert.id });
  } catch(e) {
    console.error('/api/alerts/test error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'createAlert failed — exact database error: ' + (_lastAlertError || 'none captured; see Railway logs') });
  }
});

app.get('/api/alerts', authMiddleware, async (req, res) => {
  try {
  // Honor the toggles on READ too, not just on create. Alerts created before a
  // type was turned off are still sitting in the table — this makes turning a
  // toggle off ALSO hide the ones already there, so "off means off" holds for
  // existing alerts, not just future ones.
  const settings = await getSettingsMap();
  if (settings.alerts_master === 'false') return res.json([]); // master off → show nothing, period
  const disabledTypes = Object.entries(ALERT_TYPE_TOGGLE)
    .filter(([type, key]) => settings[key] === 'false')
    .map(([type]) => type);
  let query = supabase.from('alerts').select('*').eq('resolved', false)
    .or('snoozed_until.is.null,snoozed_until.lt.' + new Date().toISOString());
  if (disabledTypes.length) query = query.not('type', 'in', '(' + disabledTypes.map(t => '"' + t + '"').join(',') + ')');
...
// Admin: resolve ALL currently-open alerts at once, and reset the stale-alert
// re-fire tracking on every job so old jobs get a genuinely clean slate — used
// for "start fresh" after changing the alert rules. It also stamps the stale
// jobs' last_stale_alert_at to now, so they won't immediately re-alert on the
// next cycle (they're within the interval again).
app.post('/api/alerts/clear-all', authMiddleware, adminOnly, async (req, res) => {
  try {
    const nowIso = new Date().toISOString();
    const { error: aErr } = await supabase.from('alerts').update({ resolved: true, resolved_at: nowIso, resolved_by: 'bulk clear', read: true }).eq('resolved', false);
    if (aErr) return res.status(500).json({ error: aErr.message });
    // Push every currently-open job's stale timer forward so nothing re-fires
    // on the very next check cycle.
    await supabase.from('jobs').update({ last_stale_alert_at: nowIso }).not('status', 'in', '("done","cancelled")').then(r => r, () => {});
    res.json({ success: true });
  } catch(e) {
    console.error('/api/alerts/clear-all error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
  ...
  const filter = req.query.filter || 'all'; // all, pending, mismatched, closed
    ...
    const enriched = (msgs || []).map(m => {
      ...
      return {
        ...
        closing_confirmed: m.closing_confirmed ?? null,
        closing_labeled_by: m.closing_labeled_by || null
      };
    });
...
app.put('/api/closing-messages/:id/review', authMiddleware, adminOnly, async (req, res) => {
  try {
  // The body may carry the admin's DECISION:
  //   confirmed = true  -> genuinely a closing
  //   confirmed = false -> false positive (a quote, chat, misread amount)
  //   omitted           -> legacy "just mark it seen" behaviour, still works
  //
  // Recording this is what turns review work into a labelled dataset. Without
  // it, confirming a real closing and dismissing a false positive were stored
  // identically, so nothing could learn from the review and the detector's true
  // accuracy could never be measured.
  // Undo: clear the label so the item returns to the pending queue. A wrong
  // label is worse than no label — it would train on a mistake — so this has
  // to genuinely erase the decision, not overwrite it with the opposite one.
  if (req.body?.reset === true) {
    const { data: cleared, error: clearErr } = await supabase.from('messages').update({
      closing_reviewed: false,
      closing_confirmed: null,
      closing_labeled_by: null,
      closing_labeled_at: null,
      closing_label_note: null
    }).eq('id', req.params.id).select().single();
    if (clearErr) return res.status(500).json({ error: clearErr.message });
    await logAudit('closing_message', req.params.id, req.user.id, req.user.name, 'label_cleared', 'Cleared closing label');
    return res.json(cleared);
  }
  const patch = { closing_reviewed: true };
  const hasDecision = typeof req.body?.confirmed === 'boolean';
  if (hasDecision) {
    patch.closing_confirmed = req.body.confirmed;
    patch.closing_labeled_by = req.user.name;
    patch.closing_labeled_at = new Date().toISOString();
    if (req.body.note) patch.closing_label_note = String(req.body.note).slice(0, 500);
  }
  const { data, error } = await supabase.from('messages').update(patch).eq('id', req.params.id).select().single();
    if (error) return res.status(500).json({ error: error.message });
    await logAudit('closing_message', req.params.id, req.user.id, req.user.name,
      hasDecision ? (req.body.confirmed ? 'confirmed' : 'rejected') : 'reviewed',
      hasDecision ? (req.body.confirmed ? 'Confirmed as a real closing' : 'Marked NOT a closing' + (req.body.note ? ': ' + req.body.note : ''))
                  : 'Marked closing message reviewed');
    res.json(data);
  } catch(e) {
...
// How good is the closing detector, really? Answered from the labels admins
// have actually applied — not an estimate. This is the number that decides
// whether automated detection is worth building at all: if precision is already
// ~95% there is little to gain, and if it's 70% there clearly is.
// Diagnostic for "appointment reminders aren't working". Reports exactly what
// the sweep sees, so we stop guessing: whether the column exists, how many jobs
// are eligible, and what the next due one is.
app.get('/api/diagnostics/reminders', authMiddleware, adminOnly, async (req, res) => {
  const out = { column_ok: null, workflows_table_ok: null, queue_table_ok: null, eligible_next_hour: 0, upcoming: [], notes: [] };
  try {
    const { error: colErr } = await supabase.from('jobs').select('appointment_reminder_sent').limit(1);
    out.column_ok = !colErr;
    if (colErr) out.notes.push('jobs.appointment_reminder_sent is MISSING — run workflow_fix.sql. Reminders cannot run without it.');
  } catch(e) { out.column_ok = false; }
  try {
    const { error } = await supabase.from('workflows').select('id').limit(1);
    out.workflows_table_ok = !error;
    if (error) out.notes.push('workflows table missing — run workflow_fix.sql.');
  } catch(e) { out.workflows_table_ok = false; }
  try {
    const { error } = await supabase.from('workflow_queue').select('id').limit(1);
    out.queue_table_ok = !error;
    if (error) out.notes.push('workflow_queue table missing — run workflow_delays.sql (delayed actions will not run).');
  } catch(e) { out.queue_table_ok = false; }
  try {
    const now = new Date();
    const in1h = new Date(now.getTime() + 3600000);
    const { data } = await supabase.from('jobs')
      .select('id,scheduled_date,tech_name,status,appointment_reminder_sent')
      .not('scheduled_date', 'is', null)
      .gte('scheduled_date', now.toISOString())
      .lte('scheduled_date', in1h.toISOString())
      .limit(20);
    const rows = data || [];
    out.eligible_next_hour = rows.filter(j => j.appointment_reminder_sent === false && j.status !== 'done' && j.status !== 'cancelled').length;
    out.upcoming = rows.map(j => ({ id: j.id, scheduled_date: j.scheduled_date, tech: j.tech_name, status: j.status, already_sent: j.appointment_reminder_sent }));
    if (!rows.length) out.notes.push('No jobs scheduled in the next hour — nothing to remind about. Schedule a test job ~30 minutes out.');
    rows.filter(j => !j.tech_name).forEach(j => out.notes.push(`${j.id} has no tech assigned — the reminder texts the TECH, so it will be skipped.`));
  } catch(e) { out.notes.push('Query failed: ' + e.message); }
  res.json(out);
});

app.get('/api/closing-messages/accuracy', authMiddleware, adminOnly, async (req, res) => {
  try {
    const { data, error } = await supabase.from('messages')
      .select('closing_confirmed')
      .eq('is_closing_attempt', true)
      .not('closing_confirmed', 'is', null);
    if (error) throw new Error(error.message);
    const rows = data || [];
    const confirmed = rows.filter(r => r.closing_confirmed === true).length;
    const rejected = rows.filter(r => r.closing_confirmed === false).length;
    const labeled = confirmed + rejected;
    res.json({
      labeled,
      confirmed,
      rejected,
      precision_pct: labeled ? Math.round((confirmed / labeled) * 1000) / 10 : null,
      // Rough guide: a text classifier needs a few hundred examples of each
      // class before it beats hand-written rules.
      ready_to_train: confirmed >= 150 && rejected >= 50
    });
  } catch(e) {
    console.error('/api/closing-messages/accuracy error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
// COMMISSION RESOLUTION — one place, one meaning.
//
// The old version took a single "subtotal" and three different callers passed
// three different things, so a rule threshold could silently be tested against
// the labor basis in one place and the full closing total in another.
//
// Now every rule states WHICH NUMBER it tests:
//   basis 'labor' (default) = closing_total − closing_parts  ... the job's profit
//   basis 'total'           = closing_total                  ... parts included
//
// Worked example: closed for $160 with $30 of parts.
//   labor = $130, total = $160.
//   Rule "if labor below 150 → 50%" MATCHES (130 < 150) even though the
//   closing total is 160. That is the intended behaviour.
//
// Rules are evaluated top to bottom, first match wins; if none match, the
// tech's default applies. The commission itself is always calculated on the
// LABOR basis — you don't pay a percentage of parts you had to buy.
function resolveCommission(tech, arg) {
  let total, parts;
  if (typeof arg === 'number') {
    // Legacy callers passed the labor basis as a bare number.
    total = arg; parts = 0;
  } else {
    total = parseFloat(arg?.total) || 0;
    parts = parseFloat(arg?.parts) || 0;
  }
  const labor = Math.max(0, total - parts);

  const rules = Array.isArray(tech?.commission_rules) ? tech.commission_rules : [];
  for (const rule of rules) {
    const amt = parseFloat(rule.amount);
    if (!isFinite(amt)) continue;                       // a malformed rule is skipped, never matched
    const basisVal = rule.basis === 'total' ? total : labor;   // default: labor
    let matches = false;
    if (rule.operator === 'below') matches = basisVal < amt;
    else if (rule.operator === 'at_or_below') matches = basisVal <= amt;
    else if (rule.operator === 'above') matches = basisVal > amt;
    else if (rule.operator === 'at_or_above') matches = basisVal >= amt;
    if (matches) {
      return {
        type: rule.type === 'cash' ? 'flat' : rule.type,
        value: parseFloat(rule.value) || 0,
        rule_applied: rule,
        basis_used: rule.basis === 'total' ? 'total' : 'labor',
        basis_value: basisVal
      };
    }
  }
  return {
    type: tech?.default_commission_type || 'percentage',
    value: parseFloat(tech?.default_commission_value) || 0,
    rule_applied: null,
    basis_used: 'labor',
    basis_value: labor
  };
...
app.put('/api/jobs/:id/closing', authMiddleware, requirePerm('perm_edit_jobs'), async (req, res) => {
  try {
    ...
    // Optional customer-facing itemization. The closing TOTAL always governs
    // what's owed — these lines only describe it on the receipt, so a mismatch
    // between the lines and the total can never change the money.
    const items = Array.isArray(req.body.items)
      ? req.body.items
          .filter(li => (li.description || '').trim() !== '' || parseFloat(li.amount))
          .map(li => ({
            description: String(li.description || '').slice(0, 120),
            amount: Math.round((parseFloat(li.amount) || 0) * 100) / 100
          }))
      : undefined;
    const { data: job, error } = await supabase.from('jobs').update({
      closing_total: total,
      closing_parts: parts,
      ...(req.body.business_name ? { business_name: req.body.business_name } : {}),
      ...(items !== undefined ? { closing_items: items } : {}),
...
// Recalculate a tech's commission on ALREADY-CLOSED jobs using their CURRENT
// commission rules. This rewrites historical financial records, so:
//   - GET (preview=1) returns the before/after WITHOUT changing anything, so
//     the admin can review before committing.
//   - POST actually applies the changes.
// Admin-only, and deliberately never touches the customer-facing total or
// parts — only the commission rate/amount (and the derived company net).
async function recalcTechCommissions(techId, apply) {
  const { data: tech } = await supabase.from('users').select('*').eq('id', techId).single();
  if (!tech) return { error: 'Tech not found' };
  const { data: jobs } = await supabase.from('jobs').select('*').eq('tech_id', techId).not('closing_total', 'is', null);
  const changes = [];
  for (const j of jobs || []) {
    const total = parseFloat(j.closing_total) || 0;
    const parts = parseFloat(j.closing_parts) || 0;
    const labor = total - parts;
    const resolved = resolveCommission(tech, { total, parts });
    const newAmount = Math.round(calcCommissionAmount(resolved.type, resolved.value, labor) * 100) / 100;
    const oldAmount = parseFloat(j.closing_commission_amount) || 0;
    const oldType = j.closing_commission_type;
    const oldValue = parseFloat(j.closing_commission_value) || 0;
    const rateChanged = oldType !== resolved.type || Math.abs(oldValue - resolved.value) > 0.001;
    const amtChanged = Math.abs(oldAmount - newAmount) > 0.001;
    if (!rateChanged && !amtChanged) continue;
    changes.push({
      id: j.id, total, parts,
      old_rate: oldType === 'flat' ? ('$' + oldValue + ' flat') : (oldValue + '%'),
      new_rate: resolved.type === 'flat' ? ('$' + resolved.value + ' flat') : (resolved.value + '%'),
      old_amount: oldAmount, new_amount: newAmount, delta: Math.round((newAmount - oldAmount) * 100) / 100
    });
    if (apply) {
      await supabase.from('jobs').update({
        closing_commission_type: resolved.type,
        closing_commission_value: resolved.value,
        closing_commission_amount: newAmount
      }).eq('id', j.id);
    }
  }
  return { tech_name: tech.name, total_closed: (jobs || []).length, changed: changes.length, changes };
}

app.get('/api/techs/:id/recalc-commissions', authMiddleware, adminOnly, async (req, res) => {
  try {
    const result = await recalcTechCommissions(req.params.id, false); // preview only
    if (result.error) return res.status(404).json(result);
    res.json(result);
  } catch(e) {
    console.error('recalc-commissions preview error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});

app.post('/api/techs/:id/recalc-commissions', authMiddleware, adminOnly, async (req, res) => {
  try {
    const result = await recalcTechCommissions(req.params.id, true); // apply
    if (result.error) return res.status(404).json(result);
    broadcastNudge('jobs_changed');
    res.json(result);
  } catch(e) {
    console.error('recalc-commissions apply error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
// Returns the receipt link WITHOUT sending anything, so a receipt can be
// reviewed exactly as the customer will see it before it goes out. The token is
// created on first use, the same one the send path uses — so what you review is
// literally what they receive, not a preview that could differ.
app.get('/api/jobs/:id/receipt-link', authMiddleware, async (req, res) => {
  try {
    const { data: job } = await supabase.from('jobs').select('id,closing_total,invoice_id').eq('id', req.params.id).single();
    if (!job) return res.status(404).json({ error: 'Job not found' });
    if (job.closing_total === null || job.closing_total === undefined) {
      if (!job.invoice_id) return res.status(400).json({ error: 'No closing or invoice on this job yet — nothing to show on a receipt.' });
    }
    // Job closing takes precedence (that's the system of record); fall back to
    // the invoice receipt when there's no closing yet.
    const token = (job.closing_total !== null && job.closing_total !== undefined)
      ? await ensureJobReceiptToken(req.params.id)
      : await ensureReceiptToken(job.invoice_id);
    res.json({ link: `${PUBLIC_BASE_URL}/receipt/${token}` });
  } catch(e) {
    console.error('/api/jobs/:id/receipt-link error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
app.post('/api/jobs/:id/send-receipt', authMiddleware, smsSendLimiter, async (req, res) => {
  try {
    ...
    // No company name in the SMS. Which business issued this appears in the
    // document header, and hard-coding it here meant a receipt could be texted
    // under one name while the document it links to showed another.
    const msg = `Hi! Here's your receipt: ${link}`;
    const finalMsg = await appendComplianceFooterIfFirstContact(phone, msg);
    const sentRcpt = await twilioClient.messages.create({ body: finalMsg, from: process.env.TWILIO_PHONE_NUMBER, to: phone });
    // Record it in the customer's conversation so the receipt send is visible
    // and provable — previously it went out with no trace in the CRM at all.
    await logOutboundToThread({
      phone, body: finalMsg, job, twilio_sid: sentRcpt?.sid,
      name: job?.customer_name || null, contact_type: 'customer',
      sent_by: `Receipt · ${req.user?.name || 'System'}`
    });
...
app.post('/api/invoices/:id/receipt', authMiddleware, async (req, res) => {
  try {
    ...
    const link = `${req.protocol}://${req.get('host')}/receipt/${token}`;
    const msg = `Hi! Here's your receipt: ${link}`;   // name lives in the document, not the text
    if (customer?.phone) {
...
// Mirrors the receipt design: a public token so the customer opens it without
// logging in, line items you control, and the same business details on both
// documents. Kept in its own table so a quote can never be counted as revenue —
// closing-first accounting is untouched.

// Small money formatter for SMS/alert copy (no Intl construction per call).
function fmtMoney(n) {
  const v = parseFloat(n);
  return isFinite(v) ? '$' + v.toFixed(2) : '$0.00';
}

// Substitutes the estimate's actual validity into the terms text.
// Supports {valid_days} and {valid_until}. If the terms contain neither
// placeholder they're shown as written — including a literal "30 days", which
// is why the default text now uses the placeholder.
function renderEstimateTerms(tpl, est) {
  if (!tpl) return '';
  let days = '';
  if (est && est.valid_until && est.created_at) {
    const from = new Date(est.created_at);
    const to = new Date(est.valid_until + 'T00:00:00Z');
    const d = Math.round((to - from) / 86400000);
    if (isFinite(d) && d > 0) days = String(d);
  }
  const until = est && est.valid_until
    ? new Date(est.valid_until + 'T00:00:00Z').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric', timeZone: 'UTC' })
    : '';
  return tpl
    .replace(/\{valid_days\}/g, days || '30')
    .replace(/\{valid_until\}/g, until);
}

function estimateTotals(items, taxRate, discount) {
  const lines = Array.isArray(items) ? items : [];
  const subtotal = lines.reduce((sum, li) => {
    const qty = parseFloat(li.qty); const price = parseFloat(li.unit_price);
    return sum + ((isFinite(qty) ? qty : 0) * (isFinite(price) ? price : 0));
  }, 0);
...
async function generateEstimateId() {
  // Same guard as job IDs: take the true numeric max, never a row count, so
  // deleting an estimate can't cause the next one to collide.
  const { data } = await supabase.from('estimates').select('id').ilike('id', 'EST-%');
  let max = 0;
  (data || []).forEach(r => {
...
app.get('/api/estimates', authMiddleware, async (req, res) => {
  try {
    let q = supabase.from('estimates').select('*').order('created_at', { ascending: false }).limit(500);
    ...
    console.error('/api/estimates error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
app.post('/api/estimates', authMiddleware, async (req, res) => {
  try {
    const b = req.body || {};
    const totals = estimateTotals(b.line_items, b.tax_rate, b.discount);
    const id = await generateEstimateId();
    const validDays = parseInt(b.valid_days) || 30;
    ...
    const { data, error } = await supabase.from('estimates').insert({
      ...
      notes: b.notes || null, internal_notes: b.internal_notes || null,
      line_items: Array.isArray(b.line_items) ? b.line_items : [],
      tax_rate: parseFloat(b.tax_rate) || 0,
      ...totals,
      status: 'draft',
    ...
    if (error) throw new Error(error.message);
    await logAudit('estimate', id, req.user.id, req.user.name, 'created', `Created estimate ${id}`);
    res.json(data);
  } catch(e) {
    console.error('POST /api/estimates error:', e.message);
...
app.put('/api/estimates/:id', authMiddleware, async (req, res) => {
  try {
    ...
    ['customer_name','customer_phone','address','city','state','zip_code','job_type','notes','internal_notes','status','valid_until'].forEach(k => {
      if (b[k] !== undefined) patch[k] = b[k];
    });
    // valid_days was accepted on create but IGNORED on edit, so changing the
    // period on an existing estimate silently did nothing.
    if (b.valid_days !== undefined && b.valid_until === undefined) {
      const days = parseInt(b.valid_days);
      if (isFinite(days) && days > 0) {
        const { data: cur } = await supabase.from('estimates').select('created_at').eq('id', req.params.id).single();
        const base = cur?.created_at ? new Date(cur.created_at) : new Date();
        patch.valid_until = new Date(base.getTime() + days * 86400000).toISOString().slice(0, 10);
      }
    }
    if (b.line_items !== undefined || b.tax_rate !== undefined || b.discount !== undefined) {
      const { data: cur } = await supabase.from('estimates').select('line_items,tax_rate,discount').eq('id', req.params.id).single();
      const items = b.line_items !== undefined ? b.line_items : (cur?.line_items || []);
      const rate = b.tax_rate !== undefined ? b.tax_rate : (cur?.tax_rate || 0);
      const disc = b.discount !== undefined ? b.discount : (cur?.discount || 0);
      patch.line_items = Array.isArray(items) ? items : [];
      patch.tax_rate = parseFloat(rate) || 0;
      Object.assign(patch, estimateTotals(patch.line_items, patch.tax_rate, disc));
    }
    const { data, error } = await supabase.from('estimates').update(patch).eq('id', req.params.id).select().single();
    if (error) throw new Error(error.message);
    res.json(data);
  } catch(e) {
    console.error('PUT /api/estimates/:id error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: e.message });
  }
});

app.delete('/api/estimates/:id', authMiddleware, adminOnly, async (req, res) => {
  try {
    const { error } = await supabase.from('estimates').delete().eq('id', req.params.id);
    if (error) throw new Error(error.message);
    await logAudit('estimate', req.params.id, req.user.id, req.user.name, 'deleted', `Deleted estimate`);
    res.json({ success: true });
  } catch(e) {
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
...
// Text the customer a link to the estimate — and log it into their thread, so
// the send is visible and provable exactly like a receipt.
app.post('/api/estimates/:id/send', authMiddleware, async (req, res) => {
  try {
    const { data: est } = await supabase.from('estimates').select('*').eq('id', req.params.id).single();
    if (!est) return res.status(404).json({ error: 'Estimate not found' });
    const phone = req.body?.phone || est.customer_phone;
    if (!phone) return res.status(400).json({ error: 'No customer phone on this estimate' });

    const settings = await getSettingsMap();
    const link = `${PUBLIC_BASE_URL}/estimate/${est.public_token}`;
    const msg = req.body?.message
      || `Here is your estimate ${est.id} for ${fmtMoney(est.total)}. View and approve it here: ${link}`;
    const finalMsg = await appendComplianceFooterIfFirstContact(phone, msg);

    const sent = await twilioClient.messages.create({
      body: finalMsg, from: process.env.TWILIO_PHONE_NUMBER, to: phone,
      statusCallback: `${PUBLIC_BASE_URL}/api/webhooks/sms-status`
    });
    await logOutboundToThread({
      phone, body: finalMsg, job: { id: est.job_id }, twilio_sid: sent?.sid,
      name: est.customer_name, contact_type: 'customer',
      sent_by: `Estimate ${est.id} · ${req.user.name}`
    });
    const { data } = await supabase.from('estimates')
      .update({ status: est.status === 'draft' ? 'sent' : est.status, sent_at: new Date().toISOString() })
      .eq('id', est.id).select().single();
    await logAudit('estimate', est.id, req.user.id, req.user.name, 'sent', `Sent estimate to ${phone}`);
    res.json({ success: true, link, estimate: data });
  } catch(e) {
    console.error('POST /api/estimates/:id/send error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: e.message });
  }
});

// Turn an accepted estimate into a real job.
app.post('/api/estimates/:id/convert', authMiddleware, async (req, res) => {
  try {
    const { data: est } = await supabase.from('estimates').select('*').eq('id', req.params.id).single();
    if (!est) return res.status(404).json({ error: 'Estimate not found' });
    if (est.converted_job_id) return res.status(400).json({ error: `Already converted to ${est.converted_job_id}` });
    const jobId = await generateJobId();
    const { data: job, error } = await supabase.from('jobs').insert({
      id: jobId,
      customer_name: est.customer_name, phone: est.customer_phone,
      address: est.address, city: est.city, state: est.state, zip_code: est.zip_code,
      job_type: est.job_type || 'Service Call', title: est.job_type || 'Service Call',
      status: 'unassigned', source: 'Estimate',
      notes: est.notes || null,
      created_at: new Date().toISOString()
    }).select().single();
    if (error) throw new Error(error.message);
    await supabase.from('estimates').update({ converted_job_id: jobId, status: 'accepted', responded_at: new Date().toISOString() }).eq('id', est.id);
    await logAudit('estimate', est.id, req.user.id, req.user.name, 'converted', `Converted to job ${jobId}`);
    broadcastNudge('jobs_changed', { id: jobId, job });
    res.json({ success: true, job });
  } catch(e) {
    console.error('POST /api/estimates/:id/convert error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: e.message });
  }
});

// ── PUBLIC estimate document (no auth — the token IS the credential) ────────
// ─── INVOICE DOCUMENTS ────────────────────────────────────────────────────────
// Same shape as estimates: you build the line items, the customer opens a clean
// document at a public link, and the send is logged into their conversation.
// The difference is what it means — an estimate is a quote to be approved, an
// invoice is money owed.

async function generateInvoiceId() {
  // Take the true numeric MAX, never a row count. The old code used
  // count + 1, so deleting any invoice made the next one collide with an
  // existing primary key — the same bug that once broke job IDs.
  const { data: settings } = await supabase.from('settings').select('value').eq('key', 'invoice_prefix').single();
  const prefix = settings?.value || 'INV';
  const { data } = await supabase.from('invoices').select('id').ilike('id', prefix + '-%');
  let max = 0;
  (data || []).forEach(r => {
    const n = parseInt(String(r.id).replace(/\D/g, ''), 10);
    if (isFinite(n) && n > max) max = n;
...
// Create or update an invoice document from the ticket's line items.
app.post('/api/invoices/build', authMiddleware, async (req, res) => {
  try {
    const b = req.body || {};
    if (!b.job_id) return res.status(400).json({ error: 'job_id is required' });
    const totals = estimateTotals(b.line_items, b.tax_rate, b.discount);   // identical money rules
    const settings = await getSettingsMap();
    const dueDays = parseInt(b.due_days ?? settings.invoice_due_days ?? 0) || 0;
    ...
    const fields = {
      ...
      tech_name: b.tech_name || null,
      line_items: Array.isArray(b.line_items) ? b.line_items : [],
      tax_rate: parseFloat(b.tax_rate) || 0,
      notes: b.notes || null,
      internal_notes: b.internal_notes || null,
      business_name: b.business_name || settings.invoice_business_name || settings.business_name || null,
      due_date: dueDate,
    ...
    if (b.id) {
      const { data, error } = await supabase.from('invoices').update(fields).eq('id', b.id).select().single();
      if (error) throw new Error(error.message);
      return res.json(data);
    }
    ...
    if (error) throw new Error(error.message);
    // Link it back to the job so the ticket can find it.
    await supabase.from('jobs').update({ invoice_id: id }).eq('id', b.job_id);
    await logAudit('invoice', id, req.user.id, req.user.name, 'created', `Created invoice ${id}`);
    broadcastNudge('jobs_changed', { id: b.job_id });
    res.json(data);
  } catch(e) {
    console.error('POST /api/invoices/build error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: e.message });
  }
});

// The link, WITHOUT sending — so an invoice can be reviewed exactly as the
// customer will see it before it goes out.
app.get('/api/invoices/:id/link', authMiddleware, async (req, res) => {
  try {
    const { data: inv } = await supabase.from('invoices').select('id,public_token').eq('id', req.params.id).single();
    if (!inv) return res.status(404).json({ error: 'Invoice not found' });
    ...
    res.json({ link: `${PUBLIC_BASE_URL}/invoice/${token}` });
  } catch(e) {
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
app.post('/api/invoices/:id/send', authMiddleware, smsSendLimiter, async (req, res) => {
  try {
    const { data: inv } = await supabase.from('invoices').select('*').eq('id', req.params.id).single();
    if (!inv) return res.status(404).json({ error: 'Invoice not found' });
    const { data: job } = inv.job_id
      ? await supabase.from('jobs').select('*').eq('id', inv.job_id).single()
      : { data: null };
    // Look everywhere a number could reasonably live, in order of reliability:
    // an explicit override, the job, the invoice's linked customer record, then
    // a customer matched on the job's phone.
    let phone = req.body?.phone || job?.phone || null;
    if (!phone && inv.customer_id) {
      const { data: cust } = await supabase.from('customers').select('phone').eq('id', inv.customer_id).single();
      phone = cust?.phone || null;
    }
    if (!phone && inv.customer_name) {
      const { data: cust } = await supabase.from('customers').select('phone').eq('name', inv.customer_name).limit(1);
      phone = cust?.[0]?.phone || null;
    }
    if (!phone) {
      return res.status(400).json({
        error: `No phone number found for invoice ${inv.id}. Add a phone to job ${inv.job_id || '(unlinked)'} and try again.`
      });
    }
    ...
    const settings = await getSettingsMap();
    const link = `${PUBLIC_BASE_URL}/invoice/${token}`;
    // The business name is deliberately absent — the invoice document itself
    // carries whichever name it was issued under.
    const msg = req.body?.message || `Invoice ${inv.id} for ${fmtMoney(inv.total)}. View it here: ${link}`;
    const finalMsg = await appendComplianceFooterIfFirstContact(phone, msg);
    ...
    await logOutboundToThread({
      phone, body: finalMsg, job: job || { id: inv.job_id }, twilio_sid: sent?.sid,
      name: inv.customer_name || job?.customer_name || null, contact_type: 'customer',
      sent_by: `Invoice ${inv.id} · ${req.user.name}`
    });
    await supabase.from('invoices').update({ sent_at: new Date().toISOString() }).eq('id', inv.id);
    await logAudit('invoice', inv.id, req.user.id, req.user.name, 'sent', `Sent invoice to ${phone}`);
    res.json({ success: true, link });
  } catch(e) {
    console.error('POST /api/invoices/:id/send error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: e.message });
  }
});

// ── PUBLIC invoice document (no auth — the token IS the credential) ─────────
app.get('/invoice/:token', (req, res) => res.sendFile(path.join(__dirname, 'public', 'invoice.html')));

app.get('/api/invoice/:token', async (req, res) => {
  try {
    const { data: inv } = await supabase.from('invoices').select('*').eq('public_token', req.params.token).single();
    if (!inv) return res.status(404).json({ error: 'Invoice not found' });
    if (!inv.viewed_at) {
      supabase.from('invoices').update({ viewed_at: new Date().toISOString() }).eq('id', inv.id).then(() => {}, () => {});
    }
    const { data: job } = inv.job_id
      ? await supabase.from('jobs').select('address,city,state,zip_code,job_type,title,customer_name').eq('id', inv.job_id).single()
      : { data: null };
    const settings = await getSettingsMap();
    // internal_notes deliberately excluded — it never leaves the system.
    res.json({
      id: inv.id,
      customer_name: inv.customer_name || job?.customer_name || '',
      address: job ? [job.address, job.city, job.state, job.zip_code].filter(Boolean).join(', ') : '',
      job_type: job?.job_type || job?.title || '',
      job_id: inv.job_id,
      line_items: inv.line_items || [],
      subtotal: inv.subtotal, discount: inv.discount,
      tax_rate: inv.tax_rate, tax_amount: inv.tax_amount, total: inv.total,
      notes: inv.notes,
      status: inv.status,
      invoice_date: inv.invoice_date || inv.created_at,
      due_date: inv.due_date,
      // The name this invoice was ISSUED under. Stored on the row, so an
      // invoice already sent to a customer keeps its name even if the default
      // is changed afterwards.
      business_name: inv.business_name || settings.invoice_business_name || settings.business_name || 'Express Lock&Key',
      business_phone: settings.business_phone || '',
      business_email: settings.business_email || '',
      business_email_2: settings.business_email_2 || '',
      business_address: settings.business_address || '',
      business_city_state_zip: settings.business_city_state_zip || '',
      business_address_2: settings.business_address_2 || '',
      business_city_state_zip_2: settings.business_city_state_zip_2 || '',
      business_license: settings.business_license || '',
      business_website: settings.business_website || '',
      // Sits under the business name on the document — e.g. "Mobile Locksmith
      // & Car Keys". Kept in Settings rather than hard-coded so the wording can
      // change without a deploy.
      business_tagline: settings.business_tagline || '',
      terms: settings.invoice_terms || ''
    });
  } catch(e) {
    console.error('/api/invoice/:token error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
app.get('/api/estimate/:token', async (req, res) => {
  try {
    const { data: est } = await supabase.from('estimates').select('*').eq('public_token', req.params.token).single();
    if (!est) return res.status(404).json({ error: 'Estimate not found' });
    if (!est.viewed_at) {
      supabase.from('estimates').update({ viewed_at: new Date().toISOString() }).eq('id', est.id).then(() => {}, () => {});
    }
    const settings = await getSettingsMap();
    // internal_notes is deliberately NOT included — it never leaves the system.
    res.json({
      id: est.id,
      customer_name: est.customer_name,
      address: [est.address, est.city, est.state, est.zip_code].filter(Boolean).join(', '),
      job_type: est.job_type,
      line_items: est.line_items || [],
      subtotal: est.subtotal, discount: est.discount,
      tax_rate: est.tax_rate, tax_amount: est.tax_amount, total: est.total,
      notes: est.notes,
      status: est.status,
      valid_until: est.valid_until,
      created_at: est.created_at,
      // Estimates can carry their own trading name (estimate_business_name).
      // Falls back to the global business_name when unset, so receipts and
      // everything else are untouched.
      business_name: settings.estimate_business_name || settings.business_name || 'Express Lock&Key',
      business_phone: settings.business_phone || '',
      business_email: settings.business_email || '',
      business_address: settings.business_address || '',
      business_city_state_zip: settings.business_city_state_zip || '',
      ...
      // change without a deploy.
      business_tagline: settings.business_tagline || '',
      // The terms are a template, not a fixed string. They used to hard-code
      // "30 days" while the header showed the real valid_until date, so an
      // estimate set to 14 days told the customer two different things.
      terms: renderEstimateTerms(settings.estimate_terms || '', est)
    });
  } catch(e) {
    console.error('/api/estimate/:token error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});

// Customer accepts or declines from the public page.
app.post('/api/estimate/:token/respond', async (req, res) => {
  try {
    const decision = req.body?.decision;
    if (decision !== 'accepted' && decision !== 'declined') return res.status(400).json({ error: 'Invalid decision' });
    const { data: est } = await supabase.from('estimates').select('*').eq('public_token', req.params.token).single();
    if (!est) return res.status(404).json({ error: 'Estimate not found' });
    if (est.status === 'accepted' || est.status === 'declined') return res.json({ success: true, status: est.status });
    const { data } = await supabase.from('estimates')
      .update({ status: decision, responded_at: new Date().toISOString() })
      .eq('id', est.id).select().single();
    // Informational only — an accepted estimate needs no dispatcher action, so
    // it must NOT land in the alert queue demanding to be handled. It's already
    // auto-resolved; you just get the notification.
    await createAlert(decision === 'accepted' ? 'estimate_accepted' : 'estimate_declined', {
      title: `Estimate ${est.id} ${decision}`,
      body: `${est.customer_name || 'Customer'} ${decision} estimate ${est.id} (${fmtMoney(est.total)}).`,
    ...
    console.error('/api/estimate/:token/respond error:', e.message);
    if (!res.headersSent) res.status(500).json({ error: 'Server error' });
  }
});
...
app.get('/api/receipt/:token', async (req, res) => {
  try {
    const { data: s } = await supabase.from('settings').select('key,value');
    const settings = {}; s?.forEach(x => settings[x.key] = x.value);
    // Resolves the name a receipt is ISSUED under: the row's own choice first,
    // so a receipt already sent keeps its name even if the default changes
    // later; then the receipt default, then the business name.
    const bizNameFor = (row) => (row && row.business_name) || settings.receipt_business_name || settings.business_name || 'Express Lock&Key';
    const biz = {
      business_name: bizNameFor(null),
      business_tagline: settings.business_tagline || '',
      business_phone: settings.business_phone || '',
      business_address: settings.business_address || '',
      business_city_state_zip: settings.business_city_state_zip || '',
    ...
    const { data: job } = await supabase.from('jobs').select('id,customer_name,title,job_type,address,job_date,closing_total,closing_items,business_name').eq('receipt_token', req.params.token).single();
    if (job && job.closing_total !== null && job.closing_total !== undefined) {
      return res.json({
        ...
        status: 'paid',
        // Shows the itemization you entered with the closing, if any. Parts
        // COST stays internal — these are customer-facing lines you wrote.
        breakdown: (Array.isArray(job.closing_items) && job.closing_items.length)
          ? job.closing_items.map(li => ({ label: li.description, amount: li.amount }))
          : null,
        ...biz,
        business_name: bizNameFor(job)
      });
    }
...
function workflowConditionMatches(condition, jobData) {
  ...
  // TECH SCOPE. `techs` is a list of tech ids this workflow applies to. Empty
  // or missing means ALL techs, so existing workflows keep working untouched.
  // Checked before the field condition because it's a hard gate: a workflow
  // restricted to Juan must never fire for anyone else, whatever else matches.
  if (Array.isArray(condition.techs) && condition.techs.length) {
    const jobTech = jobData && jobData.tech_id;
    if (jobTech === null || jobTech === undefined) return false;   // unassigned
    if (!condition.techs.map(String).includes(String(jobTech))) return false;
  }
  // Explicitly EXCLUDED techs — "everyone except Mike".
  if (Array.isArray(condition.exclude_techs) && condition.exclude_techs.length) {
    const jobTech = jobData && jobData.tech_id;
    if (jobTech !== null && jobTech !== undefined &&
        condition.exclude_techs.map(String).includes(String(jobTech))) return false;
  }

  if (!condition.field || condition.value === undefined || condition.value === '') return true;
...
// Writes an automated outbound message into the conversation so it's visible
// and provable in the chat, exactly like one a dispatcher typed. Used by
// workflows, receipts and reminders — anything the system sends on your behalf.
async function logOutboundToThread({ phone, body, job, twilio_sid, name, contact_type, sent_by }) {
  ...
  try {
    const { data, error } = await supabase.from('messages').insert({
      ...
      delivery_status: twilio_sid ? 'sent' : 'queued',
      sent_by_name: sent_by || 'System'
    }).select().single();
    if (error) throw new Error(error.message);
    broadcastNudge('new_message');
    return data;
  } catch(e) {
    console.error('logOutboundToThread failed:', e.message);
    return null;
  }
}

// Runs a workflow's actions, honouring a per-action delay. An action with
// delay_minutes > 0 is queued in workflow_queue and executed by the scheduler
// sweep, so "text the customer 30 minutes after the job is assigned" is
// expressible. Immediate actions (delay 0) run inline exactly as before.
async function scheduleWorkflowActions(actions, data, workflowId) {
  const now = Date.now();
  const immediate = [], delayed = [];
  (actions || []).forEach(a => {
    const d = parseInt(a.delay_minutes) || 0;
    (d !== 0 ? delayed : immediate).push(a);
  });
  if (delayed.length) {
    // POSITIVE delay = run this long AFTER the trigger fired.
    // NEGATIVE delay = run this long BEFORE the job's SCHEDULED time. That's a
    // different anchor entirely: "1 hour before the appointment" can't be
    // measured from the trigger, because the trigger (job created, tech
    // assigned) may have happened days earlier.
    const sched = data?.scheduled_date ? new Date(data.scheduled_date).getTime() : null;
    const rows = [];
    for (const a of delayed) {
      const mins = parseInt(a.delay_minutes) || 0;
      let runAt;
      if (mins < 0) {
        if (!sched || !isFinite(sched)) {
          console.warn(`Workflow action "${a.type}" wants to run before the appointment, but ${data?.id || 'this job'} has no scheduled_date — skipped.`);
          continue;
        }
        if (sched < now) {
          console.warn(`Workflow action "${a.type}" skipped — ${data?.id || 'job'} is already scheduled in the past.`);
          continue;
        }
        // e.g. -60 => one hour before the appointment
        runAt = sched + mins * 60000;
        // If we're already inside that window (a job scheduled 20 minutes from
        // now with a "1 hour before" rule), run at the next sweep rather than
        // dropping it — the intent was to warn, and late is better than never.
        if (runAt < now) runAt = now;
      } else {
        runAt = now + mins * 60000;
      }
      rows.push({
    ...
    if (rows.length) {
      const { error } = await supabase.from('workflow_queue').insert(rows);
      if (error) console.error('workflow_queue insert failed:', error.message);
    }
  }
  if (immediate.length) return executeWorkflowActions(immediate, data);
  return [];
}

// Runs anything in the queue that is now due. Called by the 5-minute sweep.
async function runDueWorkflowActions() {
  try {
    const { data: due, error } = await supabase.from('workflow_queue')
      .select('*').eq('status', 'pending').lte('run_at', new Date().toISOString()).limit(50);
    if (error) { console.error('workflow_queue read failed:', error.message); return; }
    for (const row of (due || [])) {
      let job = null;
      if (row.job_id) {
        const { data: j } = await supabase.from('jobs').select('*').eq('id', row.job_id).single();
        job = j;
      }
      try {
        await executeWorkflowActions([row.action], job || {});
        await supabase.from('workflow_queue').update({ status: 'done', ran_at: new Date().toISOString() }).eq('id', row.id);
      } catch(e) {
        console.error('queued workflow action failed:', e.message);
        await supabase.from('workflow_queue').update({ status: 'failed', error: e.message }).eq('id', row.id);
      }
    }
  } catch(e) { console.error('runDueWorkflowActions error:', e.message); }
}
  ...
  for (const action of (actions || [])) {
    try {
      ...
      if (action.type === 'send_sms' && toPhone) {
        const sent = await twilioClient.messages.create({ body: msg, from: process.env.TWILIO_PHONE_NUMBER, to: toPhone });
        // Workflow SMS used to go straight to Twilio and never touch the
        // messages table, so it was invisible in the conversation — the
        // customer got a text nobody in the CRM could see or prove was sent.
        await logOutboundToThread({
          phone: toPhone, body: msg, job: data, twilio_sid: sent?.sid,
          name: action.to === 'tech' ? (data.tech_name || 'Tech') : (data.customer_name || null),
          contact_type: action.to === 'tech' ? 'tech' : 'customer',
          sent_by: 'Workflow'
        });
        ...
        const sentWa = await twilioClient.messages.create({ body: msg, from: waFrom, to: waTo });
        await logOutboundToThread({
          phone: toPhone, body: msg, job: data, twilio_sid: sentWa?.sid,
          name: action.to === 'tech' ? (data.tech_name || 'Tech') : (data.customer_name || null),
          contact_type: action.to === 'tech' ? 'tech' : 'customer',
          sent_by: 'Workflow (WhatsApp)'
        });
      ...
      // Failures used to vanish into this array, which nothing read — a broken
      // workflow was indistinguishable from one that never fired. Now it's
      // logged AND raised as an alert so you find out the same day.
      console.error(`Workflow action "${action.type}" failed:`, e.message);
      results.push({ action: action.type, status: 'error', error: e.message });
      try {
        await createAlert('workflow_failed', {
          title: `Workflow action failed — ${action.type}`,
          body: `A workflow action (${action.type}) failed${data?.id ? ' on ' + data.id : ''}: ${e.message}`,
          job_id: data?.id || null, severity: 'warning'
        });
      } catch(_) {}
    }
  }
...
Be concise, direct, and professional. When executing actions, confirm what you did. If asked to do multiple things, list each action clearly.`;
    ...
    if (!aiRes.ok || aiData.error) {
      // The old code swallowed this entirely and replied with a generic
      // fallback, which made "the AI is broken" undiagnosable. Surface the
      // exact cause — the most common one being the API key not set at all.
      const detail = aiData.error?.message || aiData.error?.type || ('HTTP ' + aiRes.status);
      console.error('/api/ai-assistant Anthropic API error:', detail);
      const hint = !process.env.ANTHROPIC_API_KEY
        ? 'The AI assistant is not configured: the ANTHROPIC_API_KEY environment variable is not set in Railway. Add it under the service Variables and redeploy.'
        : 'The AI service returned an error: ' + detail;
      return res.json({ reply: hint, actions_taken: [] });
    }
      ...
      try {
        if (actionType === 'assign_tech' && params.job_id && params.tech_name) {
          ...
          await supabase.from('jobs').insert({
            ...
            phone: params.phone, address: params.address || null, status: 'unassigned', priority: 'med', job_date: todayInBusinessTz()
          });
...
app.get('/api/jobs/daily-board', authMiddleware, async (req, res) => {
  ...
  const date = req.query.date || todayInBusinessTz();
    // Optional date_to turns the single-day board into a range view
    // (job_date between date and date_to inclusive).
    let query = supabase.from('jobs').select('*').gte('job_date', date).lte('job_date', req.query.date_to || date).order('created_at');
...
app.post('/api/jobs/:id/auto-close-receipt', authMiddleware, async (req, res) => {
  try {
    ...
    // No company name here either — the receipt document carries it.
    let msg = `Hi ${job.customer_name || customer.name}! Your job is complete.`;
    if (inv) {
      const token = await ensureReceiptToken(inv.id);
      const link = `${req.protocol}://${req.get('host')}/receipt/${token}`;
    ...
    const finalMsg = await appendComplianceFooterIfFirstContact(customer.phone, msg);
    const sentAuto = await twilioClient.messages.create({ body: finalMsg, from: process.env.TWILIO_PHONE_NUMBER, to: customer.phone });
    await logOutboundToThread({
      phone: customer.phone, body: finalMsg, job, twilio_sid: sentAuto?.sid,
      name: job?.customer_name || customer.name || null, contact_type: 'customer',
      sent_by: 'Receipt · auto-sent on close'
    });
