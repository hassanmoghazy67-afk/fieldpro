...
// for active usage. 500 covers real use comfortably while blocking floods.
...
// SMS opt-in (PUBLIC, no auth) — 5 per hour per IP.
...
// SMS send (authenticated) — 80 per hour per IP.
...
// AI assistant — 20 per hour per IP. Most expensive per-request cost in the stack.
...
app.get('/api/jobs', authMiddleware, async (req, res) => {
  ...
  const data = await fetchAllRows(() => {
      ...
      if (req.user.role === 'tech' && !req.query.all) q = q.eq('tech_id', req.user.id);
      return q;
    });
    res.json(data);
  } catch(e) {
...
app.post('/api/jobs/:id/send-to-tech', authMiddleware, smsSendLimiter, async (req, res) => {
  try {
    ...
    const { data: tech } = await supabase.from('users').select('id,name,phone').eq('id', job.tech_id).single();
    ...
    const parts = [
      ...
      job.customer_name ? `Customer: ${job.customer_name}${job.phone ? ' ' + job.phone : ''}` : null,
      job.address ? `Address: ${job.address}` : null,
      job.car_year ? `Vehicle year: ${job.car_year}` : null,
      job.scheduled_date ? `Scheduled: ${new Date(job.scheduled_date).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}` : null,
      job.notes ? `Notes: ${job.notes}` : null,
      `Reply OK to confirm.`
    ].filter(Boolean);
    const body = parts.join('\n');
    const msg = await twilioClient.messages.create({ body, from: process.env.TWILIO_PHONE_NUMBER, to: tech.phone });
...
app.get('/api/analytics/dashboard', authMiddleware, adminOnly, async (req, res) => {
  ...
  const now = new Date();
    ...
    // FORMER TECHS — techs who appear on historical/imported jobs but no longer
    // have a CRM account. Their jobs still count: group by tech_name so their
    // performance shows in analytics alongside current techs.
    const crmTechIds = new Set((techs || []).map(t => t.id));
    const formerGroups = {};
    (jobs || []).forEach(j => {
      if ((!j.tech_id || !crmTechIds.has(j.tech_id)) && j.tech_name) {
        (formerGroups[j.tech_name] = formerGroups[j.tech_name] || []).push(j);
      }
    });
    const formerInvByName = {};
    (invoices || []).forEach(i => {
      if ((!i.tech_id || !crmTechIds.has(i.tech_id)) && i.tech_name && i.status === 'paid') {
        formerInvByName[i.tech_name] = (formerInvByName[i.tech_name] || 0) + (i.total || 0);
      }
    });
    Object.entries(formerGroups).forEach(([name, tJobs]) => {
      const doneN = tJobs.filter(j => j.status === 'done').length;
      techPerf.push({
        tech: { id: 'former:' + name, name: name + ' (former)', color: '#64748b', initials: name.slice(0, 2).toUpperCase(), role: 'tech', former: true },
        total: tJobs.length,
        done: doneN,
        cancelled: tJobs.filter(j => j.status === 'cancelled').length,
        inProgress: tJobs.filter(j => j.status === 'in-progress').length,
        revenue: formerInvByName[name] || 0,
        completionRate: tJobs.length ? Math.round(doneN / tJobs.length * 100) : 0
      });
    });
