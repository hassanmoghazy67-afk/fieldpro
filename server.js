...
app.post('/api/messages/send', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const { data } = await supabase.from('messages').insert({
      contact_name, contact_phone, contact_type, direction: 'out', body, twilio_sid: msg.sid, read: true,
...
app.post('/api/messages/send-mms', authMiddleware, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const { data } = await supabase.from('messages').insert({
      contact_name, contact_phone, contact_type, direction: 'out', body: body || '', twilio_sid: msg.sid, read: true,
...
// Public — no auth. Lets standalone public pages (opt-in, privacy, terms) show
// real business details without needing a login, so they never go stale.
