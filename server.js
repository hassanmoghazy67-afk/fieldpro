...
app.get('/api/users', authMiddleware, async (req, res) => {
  const { data, error } = await supabase.from('users').select('id,name,email,role,phone,color,initials,status,can_view_all_jobs,can_edit_jobs,can_view_finance,use_masking,direct_phone,default_commission_type,default_commission_value,commission_rules,perm_view_jobs,perm_edit_jobs,perm_delete_jobs,perm_view_customers,perm_view_sms,perm_view_booking,perm_view_finance_tools,perm_view_total_revenue,perm_view_reports,perm_view_workflows,perm_view_ai_assistant,perm_view_closing_messages,perm_manage_techs,perm_view_settings,created_at').order('created_at');
...
app.put('/api/users/:id', authMiddleware, adminOnly, async (req, res) => {
  const { name, role, phone, color, status, can_view_all_jobs, can_edit_jobs, can_view_finance, default_commission_type, default_commission_value, commission_rules, use_masking } = req.body;
