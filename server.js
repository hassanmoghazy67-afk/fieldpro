...
// ─── REAL AVAILABILITY (Google/Workiz-style slot booking) ─────────────────────
...
app.post('/api/book', async (req, res) => {
  ...
  // Real slot-based booking + auto-confirm enabled — skip the manual dispatch step
  // entirely and create the job immediately, the same way Workiz/Reserve with
  // Google do. Falls back to the existing manual-confirm flow otherwise.
  if (type === 'scheduled' && scheduled_time && autoConfirm) {
    ...
    await supabase.from('jobs').insert({
      ...
      job_date: todayInBusinessTz(), scheduled_date: scheduled_time, notes, source: 'Google Booking'
    });
