...
app.get('/api/analytics/dashboard', authMiddleware, adminOnly, async (req, res) => {
  ...
  const now = new Date();
    ...
    res.json({
      ...
      doneCount: weekJobs.filter(j => j.status === 'done').length,
...
app.post('/api/webhooks/voice', twilioWebhookAuth, async (req, res) => {
  ...
  try {
    ...
    const { data: customer } = await supabase.from('customers').select('name').eq('phone', From).single();
    if (customer) {
      const { data: job } = await supabase.from('jobs').select('id,tech_id,tech_name').eq('customer_name', customer.name).in('status',['assigned','in-progress']).order('created_at',{ascending:false}).limit(1).single();
      if (job?.tech_id) {
