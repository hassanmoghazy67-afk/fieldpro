...
async function createAlert(type, { title, body, job_id = null, tech_id = null, tech_name = null, severity = 'info', data = {}, dedupeWindowMinutes = 30 }) {
  // ONE alert per trigger while it stays unchecked. The old logic only deduped
  // when a job_id was present AND only within a 30-minute window — so (a)
  // tech_available alerts (tech_id, no job_id) were never deduped, and (b) once
  // the window elapsed a fresh duplicate was created even though the previous
  // one was still sitting unresolved. Both caused alerts to pile up.
  //
  // Now: if an UNRESOLVED alert with the same natural key already exists, we do
  // NOT create another one no matter how old it is — we just bump its
  // updated_at/body so it re-surfaces as the single current alert for that
  // trigger. The natural key is type + job_id when there's a job, otherwise
  // type + tech_id, otherwise just type.
