...
// The old pattern matched ANY bare 2-4 digit number, so plain zipcodes
// ("08102 we took it"), years, and phone fragments all landed in the closing
// review queue as false positives. The new rule: a message only counts as a
// closing attempt if it contains an explicit dollar amount — "$250", "250$",
// "250 $", "$1,250", decimals allowed — or a parts notation ("30p"/"30 parts")
// alongside one. A number with NO dollar sign is never a closing.
  ...
  try {
    ...
    if (tech) {
      ...
      if (job) {
        ...
        // Whole-word matching only. The old `.includes(k)` meant any message containing
        // the letter "k" ("took", "lock"…) or "ok" inside another word marked the job
        // confirmed. Now a keyword must appear as its own word (or the whole message).
        const confirmHit = CONFIRM_KEYWORDS.some(k => {
        ...
        // Only tag the stored message with the job when the message is actually
        // ABOUT the job: a confirmation, a closing attempt, or an explicit
        // mention of the job id. Ordinary chat from a tech who happens to have
        // an open job stays untagged — per Hassan: replying about JOB-5787 once
        // shouldn't stamp JOB-5787 on everything they say afterwards.
        const mentionsJobId = bodyLower.includes(String(job.id).toLowerCase());
        if (confirmHit || is_closing_attempt || mentionsJobId) {
          job_id = job.id;
        }
      }
...
app.put('/api/jobs/:id/closing', authMiddleware, requirePerm('perm_view_finance_tools'), async (req, res) => {
  try {
...
app.post('/api/webhooks/voice/tech-line', twilioWebhookAuth, async (req, res) => {
  ...
  try {
    ...
    await supabase.from('call_recordings').insert({ tech_name: 'Tech line', customer_name: req.body?.From || 'unknown', call_sid: req.body?.CallSid, direction: 'inbound' });
  } catch(e) {
...
app.post('/api/webhooks/voice/extension', twilioWebhookAuth, async (req, res) => {
  ...
  try {
    ...
    await supabase.from('call_recordings').insert({ tech_name: job.tech_name || 'Tech', customer_name: job.customer_name, call_sid: CallSid, direction: 'outbound' });
