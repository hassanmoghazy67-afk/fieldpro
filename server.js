...
app.post('/api/estimates/:id/send', authMiddleware, async (req, res) => {
  try {
    ...
    const settings = await getSettingsMap();
    // Same name the document shows, so the text and the page agree.
    const biz = settings.estimate_business_name || settings.business_name || 'Express Lock&Key';
    const link = `${PUBLIC_BASE_URL}/estimate/${est.public_token}`;
    const msg = req.body?.message
      || `${biz}: Here is your estimate ${est.id} for ${fmtMoney(est.total)}. View and approve it here: ${link}`;
    const finalMsg = await appendComplianceFooterIfFirstContact(phone, msg);
...
app.post('/api/invoices/:id/send', authMiddleware, smsSendLimiter, async (req, res) => {
  try {
    ...
    const settings = await getSettingsMap();
    // Same name the document header shows, so the text and the page agree.
    const biz = inv.business_name || settings.invoice_business_name || settings.business_name || 'Express Lock&Key';
    const link = `${PUBLIC_BASE_URL}/invoice/${token}`;
    const msg = req.body?.message || `${biz}: Invoice ${inv.id} for ${fmtMoney(inv.total)}. View it here: ${link}`;
    const finalMsg = await appendComplianceFooterIfFirstContact(phone, msg);
...
app.get('/api/receipt/:token', async (req, res) => {
  try {
    ...
    const { data: job } = await supabase.from('jobs').select('id,customer_name,title,job_type,address,job_date,closing_total,closing_items').eq('receipt_token', req.params.token).single();
